...
```{r}
##Decided by each plot above the data converged around 1000. 
...
var.thin = window(var.burn,thin=10)
plot(var.thin)
...
```{r}
#The pairs plots and parameter covariance matrix below show the relationship between the slope (b0 aka beta[1]), and the intercept (b1 aka beta[2), and the standard deviation (prec). There is a clear negative linear relationship between the slope and intercept term. This is expected becuase as the slope term increases, the intercept must decrease to fit the data properly. 
# convert to matrix
...
```{r}
#The summary statistics from the Bayesian regrssion model verses a classic linear regression model give very close (almost exact) results. The intercept value in the classic regression was 10.28, while the beta[1] value in the Bayes model was 10.311. Similarly, the slope coefficient value was 1.940, and the beta[2] from the Bayes model was 1.939. The SE value on the coefficient in the linear model was 0.067, which correlates directly with the prec value in the bayes model = 0.0669. The Bayes model shows more information about the SD and SE on these 3 parameters. 

summary(lm(data$y~data$x)) #Bayes regression summary 

summary(var.burn) #Classic linear regression summary
...
```{r}
#The "true" parametes were beta[1]=10, beta[2]=2. After running these models, the statistical model recoved the true model very well. The values of beta[1] from the analysis = 10.31, while beta[2]=1.939. These are extremely close to the true values. 
...
nsamp <- 5000
samp <- sample.int(nrow(var.mat),nsamp, replace = T)
...
```{r}
data.multi <- list(x1 = x1, x2 = x2, x3 = x1*x2, y = y, n = n)
```
...
```{r}
## specify priors
data.multi$b0 <- as.vector(c(0,0,0,0))      ## regression beta means
data.multi$Vb <- solve(diag(10000,4))   ## regression beta precisions
data.multi$s1 <- 0.1                    ## error prior n/2
data.multi$s2 <- 0.1                    ## error prior SS/2
```
...
```{r}
##Decided by each plot above the data converged around 700. 

burnin = 700                                 ## determine convergence
var.burn <- window(var.out,start=burnin)  ## remove burn-in
plot(var.burn)                             ## check diagnostics post burn-in
```
...
```{r}
#The pairs plots and parameter covariance matrix below show the relationship between the slope (b0 aka beta[1]), and the intercepts (b1 aka beta[2-4]), and the standard deviation (prec). There are clear linear relationships between the slope and intercept terms. This is expected becuase as the slope term increases, the intercept must decrease to fit the data properly. 
...
```
```{r}
#the plots below show that the data distribution for each predicted beta is centered around the "true" beta values displayed as red horizontal lines. 

plot(var.mat[,1], var.mat[,2], 
     ylab = "b1", xlab = "b0")
abline(h=b1, col = "red")

plot(var.mat[,1], var.mat[,3], 
     ylab = "b2", xlab = "b0")
abline(h=b2, col = "red")

plot(var.mat[,1], var.mat[,4], 
     ylab = "b3", xlab = "b0")
abline(h=b3, col = "red")
```
...
* Compare the fit parameters to the “true” parameters we used to generate the pseudo-data.  How well does the statistical analysis recover the true model?
```{r}
#below are the true parameters we used to generate the pseudo-data. Compared to the values in the summary table, this statistical anaysis recovers the true model almost perfectly. 
beta
summary(var.burn)
```
