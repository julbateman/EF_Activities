...
*The pattern of the initial condition uncertainty observed shows wider uncertainty in the beginning that quickly diminished to very, very low uncertainty. This is what is expected in terms of what table 1 says about endogenous predictions; considering the other variables or things that may affect the system are excluded, the model's CIs are very small.  
...
*The pattern of the model when parameter uncertainty is incorporated shows much more uncertainty than the previous forcast that only included initial conditions. The uncertainty seems to be stable throughout time as well, meaning the CI does not expand or decrease throughout time. This trend is expected; in Table 1. it states that it typically does not increase or decrease. 
...
*To partition the contribution of each parameter, you could add them all in one at a time and see how the model respondes with each parameter, or less parameters. 
...
* This model shows much higher uncertainty and heterogenity. As time moves on, the CI increases significantly and deviates from the other model predictions greatly. This driver uncertainty decreases the predictive power compared the other models, becuase after about 40 years the predictions contain a very wide range of possibilities (at time = 60, N = between 200-600). This makes sense considering it's predicting precipitation which is very variable. The CI for this predictionis expected to be different becuase it's capturing the range of prediction (that is often hard to predict times increases).
...
*The contribution of process error may decline with time becuase the model's abilities decline over time, and therefore the process error has less reliable predictions to work from. This matches expectations considering the CIs do encompass all posibilites from the other models.
...
* To improve out-of-sample predictions, I would focus on the random effects of the system that contribe to the variance by trying to really understand the similaries and differences between the known site and new site in order to see if there is a way to decline the amount of uncertainty coming from the fact that it's a new environment with similar features. As for in-sample predictions, I would focus on the driver error variance contribution becuase if the driver uncertainty decreased, so would the process error. 
...
* Forward Simulation: Gaussian 

```{r, echo=FALSE}
### settings
s <- 6             ## Focal site for forward simulation
Nmc <- 1000         ## same number of Monte Carlo draws
ylim = c(100,700)  ## set Y range on plot
N.cols <- c("black","red","green","blue","orange") ## set colors
trans <- 0.8       ## set transparancy
time = 1:(NT*2)    ## total time
...
*create a simple function to encapsulate making the plot, but change CI so its now calculating the mean and SDs
...
*Monte Carlo Error Propagation: Gaussian edition
...
## Plot run, but with new predictive intervals instead
plot.run()
mean.I = apply(N.I,2,mean)
sd.I = apply(N.I,2,sd)
ci.I.upper = (sd.I*1.96)+mean.I
ci.I.lower = mean.I-(sd.I*1.96)
N.I.ci.new = matrix(c(ci.I.lower,mean.I, ci.I.upper), nrow = 3, ncol = 30,byrow = TRUE)

ecoforecastR::ciEnvelope(time2,N.I.ci.new[1,],N.I.ci.new[3,],col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci.new[2,],lwd=0.5)
```
...
## Plot run
plot.run()
mean.IP = apply(N.IP,2,mean)
sd.IP = apply(N.IP,2,sd)
ci.IP.upper = (sd.IP*1.96)+mean.IP
ci.IP.lower = mean.IP-(sd.IP*1.96)
N.IP.ci.new = matrix(c(ci.IP.lower,mean.IP, ci.IP.upper), nrow = 3, ncol = 30,byrow = TRUE)


ecoforecastR::ciEnvelope(time2,N.IP.ci.new[1,],N.IP.ci.new[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci.new[1,],N.I.ci.new[3,],col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci.new[2,],lwd=0.5)
```
...
## Plot run
plot.run()
mean.IPD = apply(N.IPD,2,mean)
sd.IPD = apply(N.IPD,2,sd)
ci.IPD.upper = (sd.IPD*1.96)+mean.IPD
ci.IPD.lower = mean.IPD-(sd.IPD*1.96)
N.IPD.ci.new = matrix(c(ci.IPD.lower,mean.IPD, ci.IPD.upper), nrow = 3, ncol = 30,byrow = TRUE)

ecoforecastR::ciEnvelope(time2,N.IPD.ci.new[1,],N.IPD.ci.new[3,],col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.ci.new[1,],N.IP.ci.new[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci.new[1,],N.I.ci.new[3,],col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci.new[2,],lwd=0.5)
```
...
## Plot run
plot.run()
mean.IPDE = apply(N.IPDE,2,mean)
sd.IPDE = apply(N.IPDE,2,sd)
ci.IPDE.upper = (sd.IPDE*1.96)+mean.IPDE
ci.IPDE.lower = mean.IPDE-(sd.IPDE*1.96)
N.IPDE.ci.new = matrix(c(ci.IPDE.lower,mean.IPDE, ci.IPDE.upper), nrow = 3, ncol = 30,byrow = TRUE)

ecoforecastR::ciEnvelope(time2,N.IPDE.ci.new[1,],N.IPDE.ci.new[3,],col=col.alpha(N.cols[4],trans))
ecoforecastR::ciEnvelope(time2,N.IPD.ci.new[1,],N.IPD.ci.new[3,],col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.ci.new[1,],N.IP.ci.new[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci.new[1,],N.I.ci.new[3,],col=col.alpha(N.cols[1],trans))
lines(time2,N.I.ci.new[2,],lwd=0.5)
...
## Plot run
plot.run()
mean.IPDEA = apply(N.IPDEA,2,mean)
sd.IPDEA = apply(N.IPDEA,2,sd)
ci.IPDEA.upper = (sd.IPDEA*1.96)+mean.IPDEA
ci.IPDEA.lower = mean.IPDEA-(sd.IPDEA*1.96)
N.IPDEA.ci.new = matrix(c(ci.IPDEA.lower,mean.IPDEA, ci.IPDEA.upper), nrow = 3, ncol = 30,byrow = TRUE)

ecoforecastR::ciEnvelope(time2,N.IPDEA.ci.new[1,],N.IPDEA.ci.new[3,],col=col.alpha(N.cols[5],trans))
ecoforecastR::ciEnvelope(time2,N.IPDE.ci.new[1,],N.IPDE.ci.new[3,],col=col.alpha(N.cols[4],trans))
ecoforecastR::ciEnvelope(time2,N.IPD.ci.new[1,],N.IPD.ci.new[3,],col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.ci.new[1,],N.IP.ci.new[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci.new[1,],N.I.ci.new[3,],col=col.alpha(N.cols[1],trans))
...
##Repeat but with smaller Nmc 
* Forward Simulation: Gaussian 

```{r, echo=FALSE}
### settings
s <- 6             ## Focal site for forward simulation
Nmc = 60       ## reduced number of Monte Carlo draws
ylim = c(100,700)  ## set Y range on plot
N.cols <- c("black","red","green","blue","orange") ## set colors
trans <- 0.8       ## set transparancy
time = 1:(NT*2)    ## total time
...
* After transforming the Monte Carlo simulation into a Guassian ensemble, there is a very clear difference in the driver uncertainty. In the MCMC model the driver uncertainty is close on the upper quantile, but deviates significantly on the lower bound. When the guassian model was done with the calculated mean and SD confidence intervals, it made the driver uncertianty bounds more evenly distributed from the mean. This made the driver uncertainty fit into the process uncertainty bound, especially as time progresses. Overall, the guassian replication of this model still fits very well, if not better than the MCMC, especially when sampling in-sample. The random effects also shows a decrease in uncertainty, especially at the lower bound.
*After the sample size was decreased from 1000 to 60, similar differences were observed. The driver uncertainty decreased and was much tighter to the overall trend in comparison to both of the previous models. The process uncertainty was wider than both of the previous models though, which could be a result of the decreased amount of samples from this system, and therefore less certainty when sampling from it. The random error on the reduced Gaussian model was much more confident on the upper bound than the full-sized gaussian, but still had much less uncertainty on the lower bound in comparison to the MCMC. For such a large reduction of samples, this reduced gaussian model doesnt seem to perform much worse then the gaussian model with 1000 samples, and may even be better than the MCMC model. 
=======
...
>>>>>>> 34513e5b49ec023634379a78e6a2722406bf4262
