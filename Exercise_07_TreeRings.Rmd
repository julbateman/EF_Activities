...
n.iter = 20000                           ##increased to 2000 iterations
...
```{r}
##Look at histogram of fixed effects, mu ,and precision
  ## process model
  vars = (1:ncol(out))[-c(which(substr(colnames(out),1,1)=="x"),grep("tau",colnames(out)),
                          grep("year",colnames(out)),grep("ind",colnames(out)))]
  par(mfrow=c(1,1))
...
n.iter = 20000                           ##increased to 20,000 
...
##Look at histogram of fixed effects, mu ,and precision
  ...
  pairs(prec)
```
> The model that incorporated the random effects had noticably better explanatory power. The diagnostic plots showed how the data fit much better within the CI intervals (these CI intervals were also tigher than the previous model). 
...
> An additional effect that could be incorporated into this model that could chip away at unexplained variability is the location of the forest (topographically) -- this could be indicative of the anount of sunlight recievced by the forest, as well as soil type in the region that could influence the amount of nutrients available to the trees. You could also look at temperature data that can be averaged out seasonally, so you could see if seasonal temperature flucuations throughout the year had any clear effect on tree growth. I think the best covariate to include would be forest density, which could represent the tree competition, which could influence growth rates. 
...
> For additional analysis, you could plot the variables I mentioned above (sunlight, soil type/nutrients, seasonal temp) against the tree growth rate. You could also plot the year random effect against the forest density, to see if there is any relationship between the amount of trees/area and the error throughout the year within the forest.

##5. Write the JAGS code that would fit the proposed model (note: you don't have to run this model, just propose the code) 
>The code below is a proposed JAGS model incorporating forest density. 

```
##FIRST find forest density data (maybe tree/m^2 or a volumetic measurement of %solid tree/m^3... I'm not quite sure what the standard forest metric typically would be, but I'm assuming it would be the per area measurement)
...
##must add another data model for the forest density: lets call it FD
  ### Data Model: FD
  for(t in 2:nt){
    FD[i,t] <- sum(x[i,t])/*forest area*
    w[i,t] ~ dnorm(FD[i,t],tau_FD)
  }
  
  #### Process Model-- add forest density into process model 
  for(t in 2:nt){
    Dnew[i,t] <- x[i,t-1] + mu + FD + ind[i] + year[t]
    x[i,t]~dnorm(Dnew[i,t],tau_add)
  }
  ...
  #### Priors-- add new prior for forest density 
  tau_dbh ~ dgamma(a_dbh,r_dbh)
  tau_inc ~ dgamma(a_inc,r_inc)
  tau_FD ~ dgamma(a_FD, r_FD)
  tau_add ~ dgamma(a_add,r_add)
  tau_ind ~ dgamma(1,0.1)
  tau_yr  ~ dgamma(1,0.1)
  mu ~ dnorm(0.5,0.5)
