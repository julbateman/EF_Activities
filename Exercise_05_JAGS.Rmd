...
```{r}
#intitial conditions: mu = 20 because that was the indicated value of mu0 from prior data provided. I chose to do 4 chains of 10,15,20,25 to incorporate that mu0 value and possible surrounding values. I chose to sample 4000 samples (iterations). 

inits.tree <- list()
inits.tree[[1]] <- list(mu = 10)
inits.tree[[2]] <- list(mu = 15)
inits.tree[[3]] <- list(mu = 20)
inits.tree[[4]] <- list(mu = 25)


j.model.tree   <- jags.model (file = textConnection(NormalMeanN),
                             data = data.tree,
                             inits = inits.tree,
                             n.chains = 4)


jags.out.tree   <- coda.samples (model = j.model.tree,
                            variable.names = c("mu"),
                                n.iter = 4000)
```

```{r}
#trace plots and density plots of MCMC indicate that the data has converged. Since the trace plot has all 4 colors overlapping and looks like white noise, we know it has converged.

plot(jags.out.tree)
```
```{r}
#Values are below 1.01 and are identical, therefore we know the data has converged. 
gelman.diag(jags.out.tree)
```

```{r}
#the first plot shows the GBR statistic versus sample... this was used to determine which samples occured before convergence, and therefore need to be removed. After looking at this plot, I decided it was safe to say the data converged around 1000. Then, I used the burn in to remove all samples before 1000th iteration from my data. 

GBR.tree <- gelman.plot(jags.out.tree)

burnin.tree = 1000                                 ## determine convergence
jags.burn.tree <- window(jags.out.tree,start=burnin.tree)  ## remove burn-in
plot(jags.burn.tree)                             ## check diagnostics post burn-in
```

```{r}
#This shows the samples after the removal of the "burn in", and it looks like all of the chains are moving together (data converged). 


acfplot(jags.burn.tree)
```

```{r}
#Effective sample size 

effectiveSize(jags.burn.tree)
```

```{r}
#These plots are just showing how mu stabilizes as a function of sample size.


cumuplot(jags.out.tree,probs=c(0.025,0.25,0.5,0.75,0.975))
```

```{r}
#thinned version still shows the "white noise" look, indicating that the samples are moving together. 

jags.thin.tree = window(jags.burn.tree,thin=10)
plot(jags.thin.tree)
```


```{r}
#The summary table below shows the stats for the final MCMC. It shows the mean value of 16.57, which is good since our mu0 was 20. These describe the posterior data. The extremely low standard deviation and SE values telle us that there is a low amount of uncertainty around our parameter (mu). Thats good news! 

summary(jags.out.tree)
```
...
```{r}
#I modified the data to make a S0 value that I took from the task 1 posterior. Using this data, I used that to make the prior for S. (I'm not sure if you're allowed to do that, but I figured I'd give it a try since it is a "past posterior".)

NormalMeanN.new <- "
model {
  mu ~ dnorm(mu0,T) # prior on the mean 
  S ~ dnorm(S0,T)  # prior on S
  for(i in 1:N){
    X[i] ~ dnorm(mu,S) # data model
  }
}
...
```{r}
inits.tree.new <- list()
inits.tree.new[[1]] <- list(mu = 10)
inits.tree.new[[2]] <- list(mu = 15)
inits.tree.new[[3]] <- list(mu = 20)
inits.tree.new[[4]] <- list(mu = 25)
```

```{r}
j.model.tree.new   <- jags.model (file = textConnection(NormalMeanN.new),
                             data = data.tree.new,
                             inits = inits.tree.new,
                             n.chains = 4)
```
```{r}
jags.out.tree.new   <- coda.samples (model = j.model.tree.new,
                            variable.names = c("mu","S"),
                                n.iter = 4000)
```
```{r}
#These look good.... trace plots look like "white noise", implying convergence. The Density plot of S seems like it may have a longer tail on the right side coparede to mu. 

plot(jags.out.tree.new)
```
```{r}
#Looks good,.. both 1. 
gelman.diag(jags.out.tree.new)
```

```{r}
#These plots are showing that S converges much earlier than mu. Does this mean the new uncertainty in S is causing variations in mu? 

GBR.tree.new <- gelman.plot(jags.out.tree.new)
```
```{r}
burnin.tree.new = 2500                                 ## determine convergence
jags.burn.tree.new <- window(jags.out.tree.new,start=burnin.tree.new)  ## remove burn-in
plot(jags.burn.tree.new)                             ## check diagnostics post burn-in
```
```{r}

acfplot(jags.burn.tree.new)
```
```{r}
effectiveSize(jags.burn.tree.new)
```
```{r}
cumuplot(jags.out.tree.new,probs=c(0.025,0.25,0.5,0.75,0.975))
```

```{r}
jags.thin.tree.new = window(jags.burn.tree.new,thin=10)
plot(jags.thin.tree.new)
```


```{r}
summary(jags.out.tree.new)

# I'm not sure if my understanding of this just isnt great or if I did it wrong... but by these results I can see that the model did not predict the value of S that well with the prior that was given. It was very difficult to figure out what value to give the prior, and what distribution would be an appropiate fit for it. Maybe once we cover more about Bayes and priors in class it will click better with me. I would love to figure out where I went wrong here/what I'm misunderstnading. 
